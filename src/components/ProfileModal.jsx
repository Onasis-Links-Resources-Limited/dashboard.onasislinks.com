...
const ProfileModal = ({ isOpen, onClose }) => {
  const { user, updateGlobalUser } = useAuth(); // <--- Use Context
  ...
  const handleAvatarSelect = async (avatarId) => {
    ...
    // If you have an API for selected ID, call it here
  ...
  const handleProfileUpdate = async () => {
    ...
    if (response.success) {
      updateGlobalUser(response.data); // <--- Instant UI Update
    } else {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (response.success) {
      updateGlobalUser({ avatar_url: response.data.avatar_url }); // <--- Instant UI Update
  ...
  // Helper for rendering profile image
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999] p-4 backdrop-blur-md">
      <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
        
        {/* --- HEADER: Centered Avatar & Name --- */}
        <div className="p-8 pb-4 flex flex-col items-center border-b border-[#2A2A2A] relative">
          <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-[#C3110C] shadow-[0_0_20px_rgba(195,17,12,0.2)] mb-3 relative group">
            {renderProfileImage()}
            {/* Tiny edit/camera overlay on hover */}
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
          ...
          {/* 1. AVATAR SELECTION */}
          <div className="mb-6 bg-[#242424] rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-gray-300">Choose Style</p>
            ...
            <button 
              ...
              className="w-full mt-3 flex items-center justify-center gap-2 border border-dashed border-[#3A3A3A] rounded-lg py-2 text-xs text-gray-400 hover:text-white hover:border-[#C3110C] transition-colors"
            >
          ...
          <div className="space-y-3">
            <div className="bg-[#242424] rounded-xl p-4 flex justify-between items-center">
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">Full Name</p>
                {isEditing ? (
                  <div className="flex gap-2">
                    <input type="text" value={formData.first_name} onChange={(e) => setFormData({...formData, first_name: e.target.value})} className="w-20 bg-[#1A1A1A] border border-[#3A3A3A] rounded px-2 py-1 text-sm text-gray-200" />
                    <input type="text" value={formData.last_name} onChange={(e) => setFormData({...formData, last_name: e.target.value})} className="w-20 bg-[#1A1A1A] border border-[#3A3A3A] rounded px-2 py-1 text-sm text-gray-200" />
                  </div>
                ) : (
              ...
              {!isEditing ? (
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setIsEditing(false)} className="text-xs text-gray-400 hover:text-gray-300 font-medium">Cancel</button>
                </div>
              )}
            </div>
            ...
            <div className="bg-[#242424] rounded-xl overflow-hidden">
              <button onClick={() => setShowPasswordChange(!showPasswordChange)} className="w-full p-4 flex items-center justify-between text-sm text-gray-300 hover:bg-[#2A2A2A] transition-colors">
              ...
              {showPasswordChange && (
                <div className="p-4 border-t border-[#2A2A2A] space-y-3">
                  <div className="relative">
                    <input type={showCurrentPass ? "text" : "password"} value={passData.current_password} onChange={(e) => setPassData({...passData, current_password: e.target.value})} className="w-full bg-[#1A1A1A] border border-[#3A3A3A] rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-[#C3110C]" placeholder="Current Password" />
                    <button onClick={() => setShowCurrentPass(!showCurrentPass)} className="absolute right-2 top-2.5 text-gray-500 hover:text-gray-300">{showCurrentPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
                  </div>
                  <div className="relative">
                    <input type={showNewPass ? "text" : "password"} value={passData.new_password} onChange={(e) => setPassData({...passData, new_password: e.target.value})} className="w-full bg-[#1A1A1A] border border-[#3A3A3A] rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-[#C3110C]" placeholder="New Password" />
                    <button onClick={() => setShowNewPass(!showNewPass)} className="absolute right-2 top-2.5 text-gray-500 hover:text-gray-300">{showNewPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
                  </div>
