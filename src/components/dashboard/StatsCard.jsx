...
const StatsCard = ({ label, value, change, icon, color }) => {
  ...
  return (
    <div
      className={`rounded-xl p-4 sm:p-5 border shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-[#C3110C] transition-all duration-300 ${isDark ? "bg-[#1A1A1A] border-[#2A2A2A]" : "bg-white border-gray-200"}`}
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <span
          className={`w-10 h-10 rounded-xl flex items-center justify-center text-base sm:text-lg ${colorMap[color]}`}
        >
      ...
      <span
        className={`inline-flex items-center gap-1 text-xs font-semibold text-green-600 px-2 py-0.5 rounded-full mt-2 ${isDark ? "bg-green-900/20 text-green-400" : "bg-green-50"}`}
      >
